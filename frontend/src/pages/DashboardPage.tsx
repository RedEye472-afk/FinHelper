/**
 * DashboardPage — переписано на React Query хуки + decimal.js деньги.
 * Store (useFinance) теперь предоставляет только KPI и expensesByCategory.
 * Goals/Budgets/Operations берём из src/api/queries.ts.
 * Деньги форматируем через src/lib/money.ts (без parseFloat/Number).
 */
...
// categoryColors перенесены локально (раньше импортировались из mockData)
...
export function DashboardPage() {
  ...
  // React Query хуки для серверных данных
  ...
  // Budget chart: лимиты по категориям (recharts требует числа для осей)
  const budgetChart = useMemo(() => {
    if (budgets.length === 0) {
      // Fallback: показываем демо-данные, если бэкенд недоступен или данные пусты
      return [
        { name: 'Продукты', limit: 35_000 },
        { name: 'Транспорт', limit: 7_000 },
        { name: 'Рестораны', limit: 12_000 },
        { name: 'Развлечения', limit: 10_000 },
        { name: 'Одежда', limit: 15_000 },
        { name: 'Жильё', limit: 45_000 },
        { name: 'Здоровье', limit: 8_000 },
        { name: 'Связь', limit: 2_500 },
      ]
    }
  ...
  // Pie data: recharts требует числовые значения
  const pieData = useMemo(() => {
    if (expensesByCategory.length === 0) {
      // Fallback: демо-данные для круговой диаграммы, если бэкенд недоступен
      return [
        { category: 'Продукты', color: '#f59e0b', value: 35_000 },
        { category: 'Жильё', color: '#ef4444', value: 40_000 },
        { category: 'Транспорт', color: '#3b82f6', value: 7_000 },
        { category: 'Рестораны', color: '#f97316', value: 12_000 },
        { category: 'Развлечения', color: '#8b5cf6', value: 10_000 },
        { category: 'Прочее', color: '#6b7280', value: 23_000 },
      ]
    }
          ...
          >
            ...
            <div className="grid grid-cols-4 gap-3">
              {quickActions.map(({ icon: Icon, label, onClick }) => (
                <button key={label} onClick={onClick} className="flex flex-col items-center gap-2 p-3 rounded-2xl card card-hover btn-press">
            ...
            <Card className="relative overflow-hidden">
              ...
              <div className="relative">
                <div className="flex items-center justify-between mb-3">
                  <p className="text-sm font-medium" style={{color: 'var(--text-secondary)'}}>Норма сбережения</p>
                    ...
                    return (
                      <div key={op.id} className="flex items-center gap-3 py-2.5" style={{borderBottom: '1px solid var(--border-subtle)'}}>
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <p className="text-xs" style={{color: 'var(--text-tertiary)'}}>{fmtDate(op.operation_date)}</p>
                        </div>
